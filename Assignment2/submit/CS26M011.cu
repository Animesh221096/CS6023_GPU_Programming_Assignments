...
__global__ void dkernel1(int* matrix1, int* matrix2, int* result, int p, int q){
	...
	int sum = 0;
	for(int k = 0 ; k < q ; k++){
		sum += matrix2[k * r + j] * row[k];
	}
	result[i * r + j] = sum;
}

__global__ void dkernel2(int* matrix1, int* matrix2, int* result, int q){
	extern __shared__ int row[];
	int i = blockIdx.x;  //p
	int j = threadIdx.x;  //r
	int r = blockDim.x;
...
void compute(int p, int q, int r, int *h_matrixA, int *h_matrixB,
	...
	dkernel1<<<p, r, q * sizeof(int)>>>(d_matrixA, d_matrixB, d_matrixE, p, q);
    dkernel2<<<p, r, q * sizeof(int)>>>(d_matrixC, d_matrixD, d_matrixE, q);
	/* ****************************************************************** */
