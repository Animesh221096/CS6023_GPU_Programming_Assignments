                                ...
                                int H, int W)
{
    ...
    if (index < H * W)
    {
        d_gray[index] = 0.299 * (float)d_rgb[offset] + 0.587 * (float)d_rgb[offset + 1] + 0.114 * (float)d_rgb[offset + 2];
    }
                             ...
                             int Hr, int Wr)
{
    ...
    if (index < Hr * Wr)
    {
        ...
        float y1 = min(y0 + 1, (float)H - 1);
        float x1 = min(x0 + 1, (float)W - 1);

        float wy = fy - y0;
        float wx = fx - x0;

        // Correct row-major indexing for input (H x W):
        float top = d_gray[(int)y0 * W + (int)x0] * (1 - wx) + d_gray[(int)y0 * W + (int)x1] * wx;
        float bottom = d_gray[(int)y1 * W + (int)x0] * (1 - wx) + d_gray[(int)y1 * W + (int)x1] * wx;

        // Correct row-major indexing for output (Hr x Wr):
                           ...
                           int Hc, int Wc)
{
    ...
    if (index < Hc * Wc)
    {
        ...
        d_cropped[index] = d_resized[(y + offsetY) * Wr + (x + offsetX)];
    }
                                ...
                                float mean, float stdv)
{
    ...
    if(index < Hc * Wc)
    {
        ...
        d_out[index] = ((d_cropped[index] / 255.0f) - mean) / stdv;
    }
...
int main(int argc, char **argv)
{
    ...
    int NUM_BLOCKS_normalize = CEIL_DIV(Hc * Wc, BLOCK_SIZE);
    normalizeKernel<<<NUM_BLOCKS_normalize, BLOCK_SIZE>>>(d_cropped, d_out, Hc, Wc, mean, stdv);
